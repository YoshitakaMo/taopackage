...
sub new {
	...
	$ESSfile->{OPT} = 0;
	$ESSfile->{STRUCTURE} = 0;
        $ESSfile->{DEBYECONVER} = 4.803205382;
...
sub prepare : method {
        ...
        $ESSfile->{TOPFILENAME} = shift; # M.Yamada
#        print $ESSfile->{FILENAME}." print file name in ESSroutine\n";
        $ESSfile->{SPIN} = shift;
#        $ESSfile->{GJFTEMPLATE} = shift;
#        $ESSfile->{GJFTEMPLATE} = shift;
#        $ESSfile->{OPT} = shift;
#        $ESSfile->{STRUCTURE} = shift;
#        print $ESSfile->{GJFTEMPLATE}." print Gaussian input file name in ESSroutine\n";
        $ESSfile->{SPIN} ||= "Alpha";
        return;
}
